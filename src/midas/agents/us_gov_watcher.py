"""US Government Watcher using LangGraph."""
...
async def fetch_news(state: AgentState) -> AgentState:
    ...
    all_feeds = build_dynamic_feeds(US_GOV_FEEDS, WATCHER_TYPE)
    dynamic_count = sum(1 for f in all_feeds if f.get("is_dynamic", False))

    if dynamic_count > 0:
        print(f"  Using {len(US_GOV_FEEDS)} base feeds + {dynamic_count} dynamic feeds from insights")
        print(format_feedback_summary(WATCHER_TYPE))
    else:
        print(f"  Using {len(US_GOV_FEEDS)} base feeds (no dynamic feeds yet)")
